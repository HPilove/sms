...
public class SMSSender extends SMSThreadSender {
    ...
    private int batchSize, increase;
    ...
    public Vector getParameterDefinition() {
        ...
        vtReturn.add(createParameter("BatchSize", "", ParameterType.PARAM_TEXTBOX_MASK, "990",
                "Số lượng bản ghi mỗi lần quét."));
        vtReturn.add(createParameter("Increase", "", ParameterType.PARAM_TEXTBOX_MASK, "0",
                "Số lần tăng so với BatchSize."));
    ...
    public void fillParameter() throws AppException {
        ...
        batchSize = loadUnsignedInteger("BatchSize");
        increase = loadUnsignedInteger("Increase");
    ...
    public void beforeSession() throws Exception {
        ...
        String sql = " SELECT id,isdn,content,shortcode,to_char(create_time,'dd/mm/yyyy hh24:mi:ss') create_time,priority,retry,program_id" +
                     " FROM mt_queue WHERE rownum <=? AND status ='0' AND retry > 0 " +
                     getPriority() + ("".equals(sql_condition) ? sql_condition : "") +
                     " ORDER BY priority,create_time,id FOR UPDATE SKIP LOCKED";
        ...
        while (miThreadCommand != ThreadConstant.THREAD_STOP) {
            ...
            try {
                ...
                if (System.currentTimeMillis() >= nextQuery) {
                    rs = DBUtil.query(qQueue, batchSize);
                    if (!rs.isBeforeFirst()) {
                        logMonitor("No data found, try new batch-size");
                        Database.closeObject(rs);
                        rs = DBUtil.query(qQueue, increase * batchSize);
                    }
